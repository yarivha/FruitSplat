...
- **Routes run 42 to 50 waves**, up from 15 to 25. Market Run 42, Twin Gates 44,
  Orchard Snake and Zigzag Grove 46, The Long Orchard 48, Meander 50 — around
  twenty minutes, which is as long as one sitting should be. Fifty was the cap
  after playing a longer one: seventy waves came to roughly fifteen minutes of
  fruit walking before any build time on top.
  ...
  - Speed ceilings raised to 1.75 / 2.75 / 3.00 from 1.35 / 1.90 / 2.20, so the
    ramp is still working at the end of a route rather than finished at wave 26.
    They sit just above what wave 50 actually reaches, so the figure on the mode
    button is close to the truth rather than a limit nothing hits.
  - Spawn intervals now fall on two slopes: as before to 0.30s by wave 28, then
    gently on, reaching 0.19s by wave 50 against a 0.16s floor the longest route
    stops just short of. Arriving faster raises the pressure without handing the
    player any more money, which no other dial does.
  ...
  - Affordability now runs flat from wave 13 to the finish: Easy 1.8, Medium
    1.1–1.4, Hard 1.0–1.1. It used to climb from 1.3 at wave 30 to 4.9 by 70.
